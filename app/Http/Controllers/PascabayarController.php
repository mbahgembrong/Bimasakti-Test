...
class PascabayarController extends Controller
{
    ...
    {
        return view('pascabayars.index');
    }

    public function getTagihan(Request $request)
    {
        $request->validate([
            'id_pelanggan' => 'required|min:12|max:12',
        ]);
        ...
        try {
            ...
            if ($response->getStatusCode() == 200) {
                return response()->json([
                    ...
                    'data' => array_diff_key($result, array_flip(['PIN', 'UID', 'METHOD', 'SALDO_TERPOTONG', 'SISA_SALDO',  'STATUS', 'URL_STRUK', 'WAKTU', 'INFOTEKS']))
                ]);
            } else {
                return response()->json([
                    'status' => 'error',
    ...
    public function payTagihan(Request $request)
    {
        ...
        try {
            ...
            if ($response->getStatusCode() == 200) {
                return view('pascabayars.struk',)->with($result);
            } else {
